module pyth_pro_compatible::accumulator {
    use std::vector::{Self};
    use sui::clock::{Clock, Self};
    use wormhole_simple_majority::bytes20::{Self, Bytes20};
    ...
    use pyth_pro_compatible::merkle_tree::{Self};
    ...
    fun parse_price_feed_message(message_cur: &mut Cursor<u8>, clock: &Clock): PriceInfo {
        ...
        let price_info = price_info::new_price_info(
            ...
            price_feed::new(
                price_identifier,
                pyth_pro_compatible::price::new(price, conf, expo, publish_time),
                pyth_pro_compatible::price::new(ema_price, ema_conf, expo, publish_time),
            )
        );
        price_info
    }
