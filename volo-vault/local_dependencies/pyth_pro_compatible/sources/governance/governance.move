module pyth_pro_compatible::governance {
    use pyth_pro_compatible::governance_instruction;
    use pyth_pro_compatible::governance_action;
    use pyth_pro_compatible::set_governance_data_source;
    use pyth_pro_compatible::set_data_sources;
    use pyth_pro_compatible::set_stale_price_threshold;
    use pyth_pro_compatible::set_fee_recipient;
    use pyth_pro_compatible::state::{Self, State};
    use pyth_pro_compatible::set_update_fee;

    use wormhole_simple_majority::vaa::{Self, VAA};
    use wormhole_simple_majority::bytes32::Bytes32;
    ...
    // We define a custom verify_vaa function instead of using wormhole_simple_majority::governance_message::verify_vaa
    ...
    public fun verify_vaa(
        ...
        let vaa_data_source = pyth_pro_compatible::data_source::new((vaa::emitter_chain(&verified_vaa) as u64), vaa::emitter_address(&verified_vaa));

        // The emitter chain and address must correspond to the Pyth governance emitter chain and contract.
        assert!(
            pyth_pro_compatible::state::is_valid_governance_data_source(pyth_state, vaa_data_source),
