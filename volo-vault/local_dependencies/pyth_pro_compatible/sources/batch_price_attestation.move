module pyth_pro_compatible::batch_price_attestation {
    use std::vector::{Self};
    use sui::clock::{Self, Clock};

    use pyth_pro_compatible::price_feed::{Self};
    use pyth_pro_compatible::price_info::{Self, PriceInfo};
    use pyth_pro_compatible::price_identifier::{Self};
    use pyth_pro_compatible::price_status;
    use pyth_pro_compatible::deserialize::{Self};

    use wormhole_simple_majority::cursor::{Self, Cursor};
    ...
    fun deserialize_price_info(cur: &mut Cursor<u8>, clock: &Clock): PriceInfo {
        ...
        let current_price = pyth_pro_compatible::price::new(price, conf, expo, publish_time);
        if (status != price_status::new_trading()) {
            current_price = pyth_pro_compatible::price::new(prev_price, prev_conf, expo, prev_publish_time);
        };
        ...
        price_info::new_price_info(
            ...
            price_feed::new(
                price_identifier,
                current_price,
                pyth_pro_compatible::price::new(ema_price, ema_conf, expo, ema_timestamp),
            )
        )
    }
