...
module navi_liquid_staking::cert {
    use std::option::{Self};
    ...
    use sui::event;
    use navi_liquid_staking::stake_pool::{AdminCap};

    /* friend navi_liquid_staking::native_pool; */
    ...
    fun init(witness: CERT, ctx: &mut TxContext) {
        ...
        transfer::public_transfer(Metadata<CERT> {
                id: object::new(ctx),
                version: VERSION,
                total_supply: supply,
        }, ctx.sender());
    ...
    public(package) fun migrate(metadata: &mut Metadata<CERT>) {
