...
module lending_core::storage {
    ...
    // Override the market_id so tests can simulate an isolated-market Storage.
    #[test_only]
    public fun set_market_id_for_testing(storage: &mut Storage, market_id: u64) {
        if (!dynamic_field::exists_(&storage.id, MARKET_KEY {})) {
            init_main_market(storage);
        };
        let info = dynamic_field::borrow_mut<MARKET_KEY, MarketInfo>(&mut storage.id, MARKET_KEY {});
        info.market_id = market_id;
        info.is_main_market = market_id == 0;
    }
